...
def convert_length(value):
    conversion_type = combo_conversion_type.get()

    if conversion_type == "Centimeter to Meter":
...
def convert_mass(value):
    ...
    elif conversion_type == "Micrograms to gram":
        result_label.config(text="{} microg is equal to {} gm".format(value, value / 1e+6))
    elif conversion_type == "Micrograms to Kg":
...
def convert_area(value):
    ...
    if conversion_type == "Centimeter to Feet":
        result_label.config(text="{} cm² is equal to {} feet².".format(value, value / 929.0304))
    elif conversion_type == "Feet to Inches":
        result_label.config(text="{} feet² is equal to {} inches².".format(value, value * 144))
    elif conversion_type == "Sqft to Sqmtrs":
        result_label.config(text="{} sqft is equal to {} sqmtrs.".format(value, value / 10.764))
    elif conversion_type == "Sqft to Hectre / Acres":
        result_label.config(text="{} sqft is equal to {} acres.".format(value, value / 43560))
...
def convert_energy(value):
    ...
    if conversion_type == "Joule to Kilojoule": 
...
def convert_pressure(value):
    ...
    if conversion_type == "Bar to Pascal": 
        result_label.config(text="{} Bar is equal to {} P.".format(value, value * 100000))
    elif conversion_type == "Bar to Std atmosphere":
...
# GUI window creation 
root = tk.Tk()
root.title("Unit Converter(mini project)")

# creating dropdown menus
label_type = ttk.Label(root, text="Type of Conversion:")
label_type.grid(row=0, column=0, padx=10, pady=10, sticky="w")

types = ["Length", "Mass", "Temperature", "Time", "Speed", "Energy", "Pressure","Area"]
...
label_conversion_type.grid(row=0, column=2, padx=10, pady=10, sticky="w")

# conversion types corresponding to the type of conversion
conversion_type_options = {
    ...
    "Area": ["Centimeter to Feet", "Feet to Inches", "Sqft to Sqmtrs", "Sqft to Hectre / Acres"]
    
}


combo_conversion_type = ttk.Combobox(root)
combo_conversion_type.grid(row=0, column=3, padx=10, pady=10)

# Value input
label_value = ttk.Label(root, text="Enter Value:")
label_value.grid(row=2, column=1, padx=10, pady=10, sticky="w")

entry_value = ttk.Entry(root)
entry_value.grid(row=2, column=2, padx=10, pady=10)
...
# Convert button with the custom style
convert_button = ttk.Button(root, text="Convert", command=convert, style="TButton")
convert_button.grid(row=3, column=1, columnspan=2, pady=10)
